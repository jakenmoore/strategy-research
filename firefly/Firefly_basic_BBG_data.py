...
df_signal =np.where(df_ret_z >= 1,-1,0)
df_signal =np.where(df_ret_z <= -1,1,df_signal)
...
df_strat_ret['TOTAL']  = (df_strat_ret.sum(axis=1) - df_signal.astype(bool).sum(axis=1) * 0.00004) / df_signal.astype(bool).sum(axis=1)
