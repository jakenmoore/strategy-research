...
def RunStrategy(df_close, df_open,df_default_vol_values):
    ...
    #df_signal = np.where(df_rank >= 7, -1, 0)
    #df_signal = np.where(df_rank <=2, 1, df_signal)
    ...
    for col in df_vol.columns:
        df_vol[[col]] = df_vol[[col]].fillna(value=df_default_vol_values[col])
    ...
    df_long_ret = np.where((df_low < df_Long_stop_level) & (df_signal_l == 1),
                                    (df_close/df_open) - 1,
                                    (df_close/df_open)-1)
    #& (df_signal_s == -1)
    df_short_ret = np.where((df_high > df_short_stop_level) & (df_signal_s == -1),
                                    (df_close/df_open)-1,
                                    (df_close/df_open)-1)
    ...
    df_effective_close_short = np.where((df_high > df_short_stop_level) & (df_signal_s == -1),df_short_stop_level,df_close)
    df_effective_close_long = np.where((df_low < df_Long_stop_level) & (df_signal_l == 1),df_Long_stop_level,0)
    ...
    return df_strat_ret_stops, df_signal ,df_effective_close

# Run the strategy
sd_strat_result,df_signal,df_effective_close = RunStrategy(df_close, df_open, _Vol)

# Strategy statistics
print("Max DrawDown " + str(np.max(sd_strat_result['Draw_Down'])) + "%")
print("Annualised return " + str(np.mean(sd_strat_result['TOTAL']) * 250 * 22) + "%")
print("Annualised volatility " + str(np.std(sd_strat_result['TOTAL']) * np.sqrt(250 * 22)) + "%")
...
ax1.plot(sd_strat_result[['Cum_TOTAL']], color=color)
