...
df_signal_s = pd.DataFrame(data=np.where((df_rank == 8) & (df_rank.shift(1) == 8),-1,0), index=df_rank.index,columns   =df_rank.columns )
df_signal_l = pd.DataFrame(data=np.where((df_rank == 1) & (df_rank.shift(1) == 1),1,0), index=df_rank.index,columns   =df_rank.columns )

df_rank2 = df_rank.shift(1)

df_signal = pd.DataFrame(data=(df_signal_s + df_signal_l), index=df_rank.index,columns   =df_rank.columns )

#calculate the return
df_strat_ret = df_ret.shift(-1) * df_signal
df_strat_ret['TOTAL']  = df_strat_ret.sum(axis=1) - df_signal.astype(bool).sum(axis=1) * 0.00004 # costs only for the trades we do
df_strat_ret['Cum_TOTAL'] = df_strat_ret['TOTAL'] .cumsum()
df_strat_ret['DollarValue'] = df_strat_ret['TOTAL']*100000    #8 is the cost ie 4 * max(2bps or 2$)
