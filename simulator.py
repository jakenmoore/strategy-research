...
class SmaCross(Strategy):
    def __init__(self, broker, data, kwargs):
        super().__init__(broker=broker, data=data, params=kwargs)
    def init(self):
        price = self.data.Close
        bid = self.data.Bid
        ask = self.data.Ask
        self.signal = self.data.Signal
        self.maB1 = self.I(SMA, bid, 10)
        self.maB2 = self.I(SMA, bid, 20)
        self.maA1 = self.I(SMA, bid, 10)
        self.maA2 = self.I(SMA, ask, 20)

    def next(self):
        if self.signal == 1:
            if crossover(self.maB1, self.maB2):
                self.buy()
            elif crossover(self.maB2, self.maB1):
                self.sell()
        elif self.signal == -1:
            if crossover(self.maA1, self.maA2):
                self.sell()
            elif crossover(self.maA2, self.maA1):
                self.buy()